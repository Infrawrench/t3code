...
// Settled labels and ordering share the same projected server timestamp.
...
export default function SidebarV2() {
  ...
  const { pinnedThreads, activeThreads, snoozedThreads, settledThreads, snoozeNow } =
    useMemo(() => {
      // Snooze classification uses a real clock because wake times are
      // second-precise. Settlement itself is projected server state.
      void snoozeWakeTick;
      ...
      for (const thread of visible) {
        ...
        if (supportsSnooze && effectiveSnoozed(thread, { now: preciseNow })) {
          snoozed.push(thread);
          // The server clears settled state on pin and clears the pin on
          // settle, so any overlap here is only a transient event race.
    ...
    (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      void (async () => {
        ...
        const threadWorkspacePath =
          ...
          null;
        // Un-settle works on every settled row and suppresses automation
        // until real activity resets the lifecycle. Environments without the
        // settlement capability get no lifecycle items at all.
        const supportsSettlement =
          serverConfigs.get(thread.environmentId)?.environment.capabilities.threadSettlement ===
          true;
        const supportsSnooze =
                  ...
                  const renderThreadRow = (
                    ...
                    return (
                      <SidebarV2Row
                        ...
                        thread={thread}
                        variant={rowVariant}
                        // Snoozed rows wake, settled rows un-settle, and cards
                        // settle. The server owns every lifecycle transition.
                        variantAction={
                          section === "snoozed"
                            ? "unsnooze"
                            : section === "settled"
